...
import { MediaLabelInputSchema } from '../schema';
import type { MediaLabel, MediaLabelInput } from '../schema';
...
> {
  ...
  protected getCollection(): RecordService<MediaLabel> {
    return this.pb.collection('MediaLabels');
  }
  ...
  ): Promise<MediaLabelInput> {
    return MediaLabelInputSchema.parse(input);
  }
   ...
   */
  async getLatestByMedia(mediaId: string): Promise<MediaLabel | null> {
