...
import type { Task, ProcessUploadPayload, ProcessUploadResult, Upload, Media } from '@project/shared';
import { StorageBackendType, MediaType, FileType, FileSource } from '@project/shared';
...
export class TranscodeService {
  ...
  async processTask(task: Task, progressCallback: (progress: number) => void): Promise<ProcessUploadResult> {
    const payload = task.payload as ProcessUploadPayload;
    const { uploadId, provider } = payload;

    this.logger.log(`Processing upload ${uploadId} with provider ${provider}`);
    ...
    const existingMedia = await this.pocketbaseService.mediaMutator.getByUpload(uploadId);
    if (existingMedia && this.isMediaComplete(existingMedia, payload)) {
      ...
      return {
        ...
        processorVersion: (existingMedia.mediaData as any)?.processorVersion || undefined,
      };
    }

    // Resolve file path - get storage path from upload or associated file
    let storagePath = upload.externalPath;
    let storageBackend = upload.storageBackend as StorageBackendType | undefined;
    
    // If no externalPath on upload, try to get from associated file
    if (!storagePath) {
      const files = await this.pocketbaseService.fileMutator.getByUpload(uploadId, 1, 1);
      if (files.items && files.items.length > 0) {
        ...
        if (!storageBackend && file.fileSource) {
          ...
          const fileSource = Array.isArray(file.fileSource) ? file.fileSource[0] : file.fileSource;
          storageBackend = fileSource === FileSource.POCKETBASE 
  ...
  private selectStrategy(provider?: string) {
    const enableGoogleTranscoder = this.configService.get('processors.enableGoogleTranscoder');
  ...
  private isMediaComplete(media: Media, payload: ProcessUploadPayload): boolean {
    return !!(
  ...
  private async createMediaRecord(
    upload: Upload,
    result: { probeOutput: any; thumbnailPath?: string; spritePath?: string; proxyPath?: string },
    _payload: ProcessUploadPayload,
  ): Promise<Media> {
    const probeOutput = result.probeOutput;
    
    // Determine media type from probe output
    const mediaType: MediaType = probeOutput.width && probeOutput.height ? MediaType.VIDEO : MediaType.AUDIO;
    ...
    const filesByType = fileResults.reduce((acc, { type, file }) => {
      acc[type] = file;
      return acc;
    ...
    const existingMedia = await this.pocketbaseService.mediaMutator.getByUpload(upload.id);
    
    if (existingMedia) {
      // Update existing media
      return await this.pocketbaseService.mediaMutator.update(existingMedia.id, {
        mediaData,
        thumbnailFileRef: filesByType.thumbnail?.id || existingMedia.thumbnailFileRef,
