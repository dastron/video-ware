...
export class MaterializationChecker {
  ...
  async getMaterializedIds(queryHash: string): Promise<string[]> {
    ...
    const result = await this.pocketbaseService.timelineRecommendationMutator.getByQueryHash(
      queryHash,
   ...
   */
  shouldSkipDuringRegeneration(recommendation: TimelineRecommendation): boolean {
   ...
   */
  async getMaterializedByTimeline(
    timelineId: string,
    page = 1,
    perPage = 50
  ) {
  ...
  async getStatistics(queryHash?: string): Promise<{
    ...
    if (queryHash) {
      ...
      const result = await this.pocketbaseService.timelineRecommendationMutator.getList(
        1,
        1000
      );
      recommendations = result.items;
