...
export class GoogleVideoIntelligenceProcessor implements MediaProcessor {
    ...
    private async resolveToGcsUri(fileRef: string): Promise<string> {
        ...
        if (this.pb) {
            try {
                ...
                const filename = Array.isArray(upload.originalFile) ? upload.originalFile[0] : upload.originalFile;
        ...
        throw new Error(`Cannot resolve ${fileRef} to GCS URI. Ensure file is in GCS and GOOGLE_CLOUD_STORAGE_BUCKET is set.`);
    }
    ...
    async generateThumbnail(fileRef: string, config: ThumbnailConfig): Promise<string> {
        throw new Error('generateThumbnail not supported by GoogleVideoIntelligenceProcessor');
    }

    async generateSprite(fileRef: string, config: SpriteConfig): Promise<string> {
        throw new Error('generateSprite not supported by GoogleVideoIntelligenceProcessor');
    }

    async detectLabels(fileRef: string, config: DetectLabelsConfig): Promise<DetectLabelsResult> {
        console.log(`[GoogleVideoIntelligenceProcessor] Detecting labels for ${fileRef}`);
