...
import type { Task, DetectLabelsPayload, DetectLabelsResult, File as FileRecord } from '@project/shared';
...
export class IntelligenceService {
  ...
  /**
   * Process intelligence extraction task
   * Coordinates video intelligence and speech-to-text analysis
   */
  async processTask(
    task: Task,
    ...
    try {
      ...
      if (existingIntelligence && this.isIntelligenceComplete(existingIntelligence, config)) {
        this.logger.log(`Intelligence data already exists for media ${mediaId}, skipping`);
      ...
      try {
        ...
        videoIntelligenceResult = await this.videoIntelligenceStrategy.detectLabels(gcsUri, config);
        this.logger.log(`Video intelligence completed: ${videoIntelligenceResult.labels.length} labels, ${videoIntelligenceResult.objects.length} objects`);
      } catch (error) {
        this.logger.error(`Video intelligence failed: ${error instanceof Error ? error.message : String(error)}`);
      ...
      try {
        ...
        this.logger.log(`Speech transcription completed: ${speechResult.transcript.length} characters`);
      } catch (error) {
        this.logger.error(`Speech transcription failed: ${error instanceof Error ? error.message : String(error)}`);
      ...
      this.logger.error(`Intelligence processing failed for media ${mediaId}: ${errorMessage}`);
   ...
   */
  private async getExistingIntelligence(mediaId: string): Promise<IntelligenceData | null> {
    try {
      // Get the latest media label record for this media
      const mediaLabel = await this.pocketbaseService.mediaLabelMutator.getLatestByMedia(mediaId);
      if (!mediaLabel) {
        return null;
      }
      ...
      this.logger.warn(`Failed to get existing intelligence data: ${error instanceof Error ? error.message : String(error)}`);
      return null;
    }
  }
   ...
   */
  private isIntelligenceComplete(data: IntelligenceData, config: DetectLabelsPayload['config']): boolean {
    // Check if required data exists based on config
    if (config.detectLabels !== false && (!data.labels || data.labels.length === 0)) {
      return false;
    }

    if (config.detectObjects !== false && (!data.objects || data.objects.length === 0)) {
      return false;
    }
   ...
   */
  private async ensureGcsUri(filePath: string, file: FileRecord): Promise<string> {
    ...
    const bucket = this.configService.get<string>('storage.s3Bucket') || 'default-bucket';
   ...
   */
  private async storeIntelligenceData(mediaId: string, data: IntelligenceData): Promise<void> {
    try {
      ...
      const existingLabel = await this.pocketbaseService.mediaLabelMutator.getLatestByMedia(mediaId);
      ...
      if (existingLabel) {
        ...
        await this.pocketbaseService.mediaLabelMutator.update(existingLabel.id, intelligenceData);
