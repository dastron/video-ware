...
export class RenderService {
  ...
  /**
   * Process a render task using the new flow-based architecture
  ...
  async processTask(task: Task): Promise<string> {
    ...
    this.logger.log(
      `Creating render flow for task ${task.id}, timeline ${payload.timelineId}`
    );
  ...
  async processTaskLegacy(
    ...
    this.logger.log(
      `Processing render task ${task.id} for timeline ${timelineId} version ${version}`
    );
    ...
    try {
      ...
      const outputPath = path.join(
        tempDir,
        `timeline_${timelineId}_v${version}.${outputSettings.format}`
      );
  ...
  ): Promise<Map<string, { media: Media; filePath: string }>> {
  ...
  private async createFileRecord(
    ...
    formData.append(
      ...
      `${timeline.name}_render.${outputSettings.format}`
    );
  ...
  private async cleanupTempFiles(tempDir: string): Promise<void> {
    try {
      ...
      this.logger.warn(
        `Failed to cleanup temp directory ${tempDir}: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
}
