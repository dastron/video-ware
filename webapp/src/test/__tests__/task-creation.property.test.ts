...
function isValidProcessUploadPayload(
  ...
  if (typeof p.originalFileRef !== 'string' || p.originalFileRef.length === 0) {
    return false;
  }
  ...
  describe('Property 7: Task Creation on Upload Completion', () => {
    ...
    it('should reject invalid payload structures', () => {
      const invalidPayloads = [
        ...
        { uploadId: 'test', originalFileRef: '' }, // Empty originalFileRef
        { uploadId: 'test', originalFileRef: 'test', provider: 'invalid' }, // Invalid provider
        {
          uploadId: 'test',
          originalFileRef: 'test',
          sprite: { fps: 'invalid' },
        }, // Invalid sprite
        {
          uploadId: 'test',
