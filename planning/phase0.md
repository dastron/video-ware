...
- Core collections exist in PocketBase (Upload, File, Media, MediaClip, Labelclips, Task) with migrations checked in.
...
- Collections: add schema definitions and migrations for Upload, File, Media, MediaClip, Labelclips, Task (fields per overview).
...
Goals: configure FFmpeg, Google Cloud Transcoder + Video Intelligence + Speech-to-Text, and S3-compatible storage; define PocketBase collections (Upload, File, Media, MediaClip, Labelclips, Task); scaffold a worker that can enqueue and process tasks, updating Task.status/progress.
